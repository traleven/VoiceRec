...
extension SecondaryButton {
    init(style: Style, action: @escaping @MainActor () -> Void, label: S, icon: String, role: Role = .regular) {
        self.init(style: style, action: action, label: label, icon: Image(systemName: icon), role: role)
    }
}

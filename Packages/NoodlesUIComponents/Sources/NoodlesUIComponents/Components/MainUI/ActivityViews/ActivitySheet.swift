        ...
        VStack(alignment: .center, spacing: 0) {
            ...
            PlainList(style: style) {
                ForEach(items, id: \.self) { item in
                    ListItem(style: style, title: item, avatar: Image(systemName: "person.circle"), content: .check(selection(for: item)))
                }
            }
            .padding(.vertical, 12)
            ...
            if selection.isEmpty {
                Activitybar.ActivityList(style: style)
                    .transition(.asymmetric(insertion: .move(edge: .trailing), removal: .move(edge: .trailing)))
            } else {
                Activitybar.Button(style: style)
...
#Preview {
    ActivitySheet(style: .standard, items: ["Chris Peng", "Andrew Lin", "Susie Sha", "Vivian Law", "Ivan Dolgushin", "Ryan Wilke"])
}

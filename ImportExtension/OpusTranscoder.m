...
int transcodeOpus(const char* inputFile, const char* outputFile) {
	...
	duration=0;
	output_seekable=fseek(output,0,SEEK_CUR)!=-1;
	if(op_seekable(of)){
		opus_int64  size;
	...
	if(!fwrite(wav_header,sizeof(wav_header),1,output)){
		...
		for(;;){
			...
			if(!fwrite(out,sizeof(*out)*4*ret,1,output)){
